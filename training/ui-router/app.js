/* myApp module */
angular.module('app', ['ui.router', 'accountSettings'])
...
angular.module('accountSettings', ['ui.router'])
    .config(['$stateProvider', '$urlRouterProvider', 
        ...
        var settings = {
            ...
            template: 
                '<div class="row"><div class="span3"><div class="pa-sidebar well well-small">'+
                  ...
                  '</ul><hr>'+
                  '<strong>named ui-view \'menu\'</strong>'+
        ...
        var userBase = {
            ...
            template: 
                ...
                '<div><label>Name</label><input type="text" class="form-control" ng-model="user.name" /></div>'+
                '<div><label>Email</label><input type="text" class="form-control" ng-model="user.email" /></div>'+
            ...
            views: {
                '': {
                    template: '<h3>{{user.name}}\'s Quotes</h3><hr>'+
                              '<div><label>Quotes</label><textarea class="form-control" type="text" ng-model="user.quotes"></textarea></div>'+
                              '<button class="btn" ng-click="done()">Save</button>'
                },
                'hint@': {
    ...
    .controller('SettingsLayoutController', ['$scope', '$stateParams', function ($scope, $stateParams) {
        $scope.user = {
            name: "Dheeraj",
            email: "dheeraj@accolite.com",
            password: 'semi-secret',
            quotes: "Accolite - Only the best technical minds"
        };
    }])
